...
**Zenist** 是一款專為 Windows 打造，專注於極簡與離線優先的待辦事項管理工具。
...
* **背景執行**：無縫支援常駐系統匣，隨點隨喚醒，不佔用工具列空間。
* **單一實例防護**：完美控制系統資源，防止軟體被重複啟動（多開）。
* **離線優先 (Offline-First)**：搭配極速的本機端資料庫，無需網路即可獲得最流暢的操作體驗，並在連線時於背景安靜同步。
* **純淨解除安裝**：安裝與解除安裝過程皆具備智能防護，自動清理登錄檔與強制關閉背景程序，不留垃圾。

## 🛠️ 技術架構
Zenist 採用現代化、高效的跨平台技術棧開發：
...
### 下載安裝
請前往 [Releases](https://github.com/ChiesiMario/Zenist/releases/latest) 頁面下載最新版本的 Windows 安裝檔 (`.exe`)。

### 從原始碼編譯
如果您希望親手編譯 Zenist 或是參與開源貢獻：

1. 請確保您的電腦已經安裝好 [Flutter](https://docs.flutter.dev/get-started/install/windows) 開發環境。
2. 複製此儲存庫：
   ```bash
...
5. 編譯成 Windows 執行檔：
   ...
   ```
*(若要製作安裝檔，您需要安裝 Inno Setup 並編譯 `windows/zenist_installer.iss` 腳本)。*
